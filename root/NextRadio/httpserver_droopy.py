...
-->
      <input value="%(update)s" onclick="swap()" type="submit">
      </p>%(station)s
    </form>
  </div>
...
en = {"maintitle":       "NextRadio Playstation List",
      "submit":          "Send",
      "sending":         "Sending",
...
zh_cn = {
      "maintitle":       u"NextRadio 网路电台播放清单",
      "submit":          u"发送",
      "sending":         u"发送中",
...
zh_tw = {
      "maintitle":       u"NextRadio 網路電台播放清單",
      "submit":          u"上傳",
      "sending":         u"傳送中...",
...
#=============================================================================
def LoadRadioList():
    filename="/root/NextRadio/upload/radio.txt"
        #t="????"
        #t+="<input name=st1 size=80 value=http://bcr.media.hinet.net/RA000072>"
        
    i = 0
    v="--- radio.txt --- </p>"
    stationlist=[]
    fd = open (filename,"r")
    v+="<textarea name=station rows=40 cols=80>"
    for line in fd:
        #sys.stdout.write('.')
    ...
    #print "Load %d items, last=%s" %(i, stationlist[max_stations-1])
    return v
#=============================================================================
...
def parse_args(cmd=None):
    ...
    """
    global picture, message, port, directory, force, must_save_options, publish_files
